...
@section('content-admin')
    @include('partials.breadcumb', ['page' => 'Quản lý Routes', 'key' => 'Index', 'link' => route('quanlyroutes.index')])
    <div class="h-12"></div>

    <div class="flex">
        <div class="mr-8 r-3">
            <a href="{{ route('quanlyroutes.create') }}" class="btn-primary">
                Thêm Routes
            </a>
        </div>
        @if (session('success'))
                        ...
                        @foreach ($route as $r)
                            <tr
                                ...
                                <th scope="row"
                                    ...
                                    {{ $r->role_id == '1' ? 'Admin' : ($r->role_id == '2' ? 'Thư ký khoa' : 'Giảng viên') }}
                                </th>
                                <td class="px-6 py-4">
                                    {{ $r->route_title }}
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('quanlyroutes.blocking', [$r->role_id, $r->route_id]) }}"
                                        class="font-medium text-blue-600 dark:text-blue-500 hover:underline">{{ $r->status == '1' ? 'Chặn' : 'Bỏ chặn'}}</a> |

                                    <span onclick="event.preventDefault(); document.getElementById('popup-modal-{{$r->role_id.$r->route_id}}').classList.add('block'); document.getElementById('popup-modal-{{$r->role_id.$r->route_id}}').classList.remove('hidden');"
                            ...
                            <div class="relative p-4 w-full max-w-md h-full md:h-auto mx-auto">
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-44 animate-fadeInDown">
                                    ...
                                    <div class="flex justify-end p-2">
                                        <span onclick="event.preventDefault(); document.getElementById('popup-modal-{{$r->role_id.$r->route_id}}').classList.add('hidden'); document.getElementById('popup-modal-{{$r->role_id.$r->route_id}}').classList.remove('block');"
